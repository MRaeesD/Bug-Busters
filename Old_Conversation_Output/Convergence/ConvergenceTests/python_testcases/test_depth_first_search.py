...
def depth_first_search(startnode, goalnode):
    ...
    def search_from(node):
        if node in nodesvisited:  # Check if the node has already been visited
            return False
        nodesvisited.add(node)  # Mark the node as visited immediately
        if node == goalnode:  # Check if the current node is the goal
            return True
        else:
            return any(
